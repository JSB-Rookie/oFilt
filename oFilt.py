...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, obj=None, **kwargs):
        ...
        #self.leDays.clearFocus.connect(self.toZeros)
    ...
    def loadPrices(self):
        ...
        try:
            self.dfP = pd.read_csv(fileName)
            print("dfP is loaded")
            self.readflag = True
            #Extrcting the ticker from the file name 
            volTicker = ""
            for element in self.leFileName.text():
                if (element.isalpha()) and (element.isupper()):
                    volTicker = volTicker + element
                    
            print("volTicker is: ", volTicker)
            self.leTicker.setText(volTicker)
            x = self.dfP[volTicker].count()
            self.lbMax.setText(str(x))
            self.leDays.setText('0')
            self.leResult.setText('0%')
        except:
            QMessageBox.about(self, "Incorrect File Name", \
                              "You must enter the correct file name!")
    ...
    def calcVol(self):
        hit = False
        z = self.readflag
        print("self.readflag is:", z)
        if self.readflag == False:
            fileName = "PriceHistory.csv"
            self.dfP = pd.read_csv(fileName)
            self.readflag = True
        if self.readflag == True:
            volTicker = self.leTicker.text()
            #x = len(self.dfP[volTicker])
            #print("The number of elements in APPL is: ", x)
            mylist = self.dfP.columns.values.tolist()
            print(volTicker)
            print(mylist)
            for each in mylist:
                print(each)
                if each == volTicker:
                    hit = True
                    if len(self.leDays.text()) == 0 or (self.leDays.text()) == '0':
                        QMessageBox.about(self, "Days Number not Entered", \
                                          "Must enter a number of days!")      
                    numdays = self.leDays.text()
                    numdays = int(numdays)
                    x = self.dfP[volTicker].count()
                    self.lbMax.setText(str(x))
                    if numdays > len(self.dfP[volTicker]):
                        QMessageBox.about(self, "Excess Number of Days", \
                                          "Must enter a smaller number of days!")    
                    na = self.dfP[[volTicker]]
                    naslice = na[0:numdays]
                    print(naslice)
                    nasd = np.std(naslice)
                    print("std of slice")
                    nsr = math.sqrt(252/numdays)
                    print("Squareroot of 252 over days is: ", nsr)
                    finc = nasd * nsr
                    finc = str(finc)
                    print(finc)
                    mylist = finc.split()
                    ...
                    fin = round(fin,2)
                    fin = str(fin)
                    self.leResult.setVisible(True)
                    self.lbResult.setVisible(True)
                    self.leResult.setText(fin + "%")
            if hit == False:
                self.lbMax.setText('0')
                QMessageBox.about(self, "No Data", "No data for the entered ticker!")
    ...
    def toZeros(self):
        self.leDays.setText("00")
        
    def graStrVol(self):
        #For eDate1
        if ((self.rbCall.isChecked()) and (self.rbWeekly.isChecked() or self.rbMonthly.isChecked())):
            print("cdf rows: ", self.cdf.shape[0])
            #xl is the list made from eDate1 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate1:
                    xl.append(self.cdf.at[ind, 'Strike'])
                       
            #x contains the x axis strike values
            x = xl
            
            #Now will create values for the y axis            
            #yl contains the list of volatility values
            yl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate1:
                    yl.append(self.cdf.at[ind, 'VolatBid'])
             
            #Because the y list contains % string values ylf will now contain floats
            ylf= []
            #Will now change the % string values to string values with no % sign
            for i in yl:
                i = i.rstrip("%")
                i = float(i)
                ylf.append(i)
            #y now contains the float values for the y axis 
            y = ylf
            
            #ylp is the list of percentages made for the y axis
            ylp = []
            
            #Convert floats to percentages
            for i in ylf:
                #Must divide vlf values by 100 to get proper values on y axis
                i = format(i/100, '.2%')
                ylp.append(i)
            ...
            #Print the date in the graph
            plt.text(100, 590., self.eDate1, fontsize = 15, transform=None)
            
            plt.bar(x, y, color='maroon', width = 0.4)
            plt.xlabel("Strikes - Calls")
            plt.ylabel("Volatilities of Bids - Calls")
            plt.title("Volitilities of Each Strike - Calls")
        
            plt.show()
            
        #For eDate1    
        elif ((self.rbPut.isChecked()) and (self.rbWeekly.isChecked() or self.rbMonthly.isChecked())):
            #xl is the list made from eDate1 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate1:
            ...
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate1:
                    yl.append(self.cdf.at[ind, 'VolatBid_1'])
            ...
            plt.xlabel("Strikes - Puts")
            plt.ylabel("Volatilities of Bids - Puts")
            plt.title("Volitilities of Each Strike - Puts")
            
            plt.show()
        
        #For calls with edate2 as maturity date
        if ((self.rbCall.isChecked()) and (self.rbWeekly.isChecked() or self.rbMonthly.isChecked())):
            print("cdf rows: ", self.cdf.shape[0])
            #xl is the list made from eDate1 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate2:
                    xl.append(self.cdf.at[ind, 'Strike'])
            ...
            plt.figure(figsize = (7, 7))
            #Print the date in the graph
            plt.text(100, 590., self.eDate2, fontsize = 15, transform=None)
            plt.bar(x, y, color = 'maroon', width = 0.4)
        ...
        #For eDate2
            ...
            #xl is the list made from eDate2 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate2:
                    xl.append(self.cdf.at[ind, 'Strike'])
        ...
        #For eDate3
        if (self.rbCall.isChecked()) and (self.rbWeekly.isChecked()):
            print("cdf rows: ", self.cdf.shape[0])
            #xl is the list made from eDate1 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate3:
                    xl.append(self.cdf.at[ind, 'Strike'])
            ...
            plt.figure(figsize = (7, 7))
            #Print the date in the graph
            plt.text(100, 590., self.eDate3, fontsize = 15, transform=None)
            plt.bar(x, y, color = 'maroon', width = 0.4)
        ...
        #For eDate3
        elif (self.rbPut.isChecked()) and (self.rbWeekly.isChecked()):
            #xl is the list made from eDate2 options 
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate3:
        ...
        if (self.rbCall.isChecked()) and (self.rbWeekly.isChecked()):
            ...
            xl = []
            for ind in self.cdf.index:
                if self.cdf.at[ind, 'ExpDate'] == self.eDate4:
                    xl.append(self.cdf.at[ind, 'Strike'])
            ...
            plt.figure(figsize = (7, 7))
            #Print the date in the graph
            plt.text(100, 590., self.eDate4, fontsize = 15, transform=None)
            plt.bar(x, y, color = 'maroon', width = 0.4)
